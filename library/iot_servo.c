...
static uint32_t calculate_duty(ledc_mode_t speed_mode, float angle) {
  float angle_us =
      angle / g_cfg[speed_mode].max_angle * (g_cfg[speed_mode].max_width_us - g_cfg[speed_mode].min_width_us) +
...
static float calculate_angle(ledc_mode_t speed_mode, uint32_t duty) {
  ...
  float angle =
      angle_us * g_cfg[speed_mode].max_angle / (g_cfg[speed_mode].max_width_us - g_cfg[speed_mode].min_width_us);
...
esp_err_t iot_servo_init(ledc_mode_t speed_mode, const servo_config_t *config) {
  ...
  SERVO_CHECK(config->freq <= SERVO_FREQ_MAX && config->freq >= SERVO_FREQ_MIN, "Servo pwm frequency out the range",
              ESP_ERR_INVALID_ARG);
  ...
  for (size_t i = 0; i < config->channel_number; i++) {
    ledc_channel_config_t ledc_ch = {.intr_type = LEDC_INTR_DISABLE,
